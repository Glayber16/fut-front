...
export default function Home() {
  ...
  const fetchMatches = async () => {
    try {
      ...
      const partidasFormatadas: Match[] = data.map((partida: any) => ({
        id: partida.id,
        date: new Date(partida.data).toLocaleDateString('pt-BR', {
          timeZone: 'UTC',
        }),
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-slate-950 text-white">
      <Sidebar />
      <main className="h-full flex-1 overflow-y-auto scroll-smooth p-4 sm:p-8">
        <h1 className="mb-6 text-2xl font-bold">Minhas Partidas</h1>

        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 2xl:grid-cols-3">
          {matches.map((partida) => (
            <MatchCard
              onDelete={() => handleDelete(partida.id)}
              key={partida.id}
              match={partida}
            />
          ))}
